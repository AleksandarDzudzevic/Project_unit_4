...
|---------------|----------|-------------------------------|--------------|
|1|The social network website will provide a secure login and registration system.|In order to meet the criteria, I needed a secure login and registartion system that would be integrated and would allow to both add new users and recognize and disitinguish existing ones. So I used sha 256 encryption, user cookies, and JWT session tokens to meet the safety criteria, and queries and database manipulation to successfully allow new users and recognization of the old ones|As it can be seen in Fig__ I have met client's criteria with this login and registartion feature. This was seen in the user interview Fig ___|
|2|The social network website will allow users to post reviews of the local places, containing title, date, and content.| To allow them to post reviews I needed to make an algotithm that would use queries and post extracted data user inputed into database containing all the posts. Then I created a HTML interface with boxes for each data set needed for a full user post |As it can be seen in Fig__ I have met client's criteria with this login and registartion feature. This was seen in the user interview Fig __|
|3|The social network website will have a feature to display all users, their contact email, and the city about which they post.|The procedure to meet his criteria was to cerate a table that would extract all data from the user database and visually present it. This was done by using an algortihm that has a query that extracts all data and then puts in the html table created for this webpage.|As it can be seen in Fig__ I have met client's criteria with this user list feature that shows all users and their contact email and city. This was seen in the user interview Fig __|
|4|The social network website will have an option to choose a specific city which you are interested in through a filter or use a username search filter, and see only relevant content creators.|TO create these two filters I needed to construct two algorithms: (1) which would use a generilzed vrsion of the city option bar and through a query find all users from the database that are fro that same city. (2) which would use a string inputed in the search bar and see all the usernames from the uer database that contain that string without being case sensitive|As it can be seen in Fig__ I have met client's criteria with this interactive city feature. This was seen in the user interview Fig ___||
|5|The social network website will to have a feature to see all reviews posted by a specific user, by selecting their profile.| I needed a way to select a specific user and then create a page template for that same user with his account name and posts. I did this by adding a hyperlink that redircts. user to a new page when they click username of a certain account. The new page will be the profile page of the account whose username was previoslu clicked and it will have all his posts| As it can be seen in Fig__ I have met client's criteria with this feature that allowed to select a certain profile and looking at their reviews. This was seen in the user interview Fig __|
|6|The social network website will have an interactive option display available cities which user could explore, with an option to learn about each one, by simply clicking at the picture of it.|(1) a general webpage that contains all cities offered in Citio and allow users to learn about each specifically by clciking on the picture which links to the second webpage (2) of a specific city that the user chose. There one could learn about the city, and also hear a fun fact about each of them|As it can be seen in Fig__ I have met client's criteria with this interactive feature to learn about cities offered in Citio. This was seen in the user interview Fig __|
