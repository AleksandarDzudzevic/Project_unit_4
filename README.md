...
|4|The social network website will have an option to choose a specific city which you are interested in through a filter or use a username search filter, and see only relevant content creators.|To create these two filters I needed to construct two algorithms: (1) which would use a generilzed vrsion of the city option bar and through a query find all users from the database that are fro that same city. (2) which would use a string inputed in the search bar and see all the usernames from the uer database that contain that string without being case sensitive|As it can be seen in Fig. 23 I have met client's criteria with this interactive city feature. This was seen in the user interview Fig. 29|
